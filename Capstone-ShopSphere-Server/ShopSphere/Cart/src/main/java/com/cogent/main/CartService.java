...
public class CartService
{
	...
	public CartEntity addItem(int userId, int num) {
		...
		if (cartItemRepository.findByUserIdAndProductId(userId, num).isPresent()) {
			CartItem cartItem = cartItemRepository.findByUserIdAndProductId(userId, num)
					.get();
			cartItem.setQuantity(cartItem.getQuantity() + 1);
			cartItemRepository.save(cartItem);

		} else { // Adds item to cart if doesn't exist
	...
	public CartEntity updateCart(int userId, List<CartItem> cartItems) {
		CartEntity cartEntity = cartRepository.findById(userId).get();

		cartItemRepository.deleteAll(cartItemRepository.findByCartEntity(cartEntity).get());

		cartItems.stream()
				.peek(c -> c.setCartEntity(cartEntity))
				.forEach(c -> cartItemRepository.save(c));

		CartEntity updatedCartEntity = cartRepository.findById(userId).get();
